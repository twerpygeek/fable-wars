...
const WORLD_CREATURES: Record<string, CreatureCfg> = {
  ...
  scorch_storm_anvil: { arch: 'machine', base: '#393238', accent: '#ffcf3c', size: 35, magnets: true },
  ...
  tide_prism_array: { arch: 'machine', base: '#d9e7e9', accent: '#39d8ef', size: 35, magnets: true },
  ...
  verdant_bog_skiff: { arch: 'machine', base: '#3c5833', belly: '#66df70', accent: '#d8f277', size: 34, magnets: true },
  verdant_mangrove_colossus: { arch: 'machine', base: '#4b3a2a', belly: '#77df6c', accent: '#d8f277', size: 44, magnets: true, big: true },
};
...
function drawMachine(ctx: Ctx, cfg: CreatureCfg, frame: number, playerHex: string, key: string): void {
  ...
  if (cfg.magnets) {
    ...
    // Tide harvesters: crab-like hauler with a crystal basket.
    const cy = feetY - 12;
    ctx.fillStyle = shade(cfg.base, -0.25);
    for (const s of [-1, 1]) {
...
function drawTerrain(t: Terrain, variant: number, withProps = true): HTMLCanvasElement {
  ...
  switch (t) {
    ...
    case Terrain.CRYSTAL: {
      fillBase('#453a31', '#241f20');
      speckle(['#6b5648', '#2b2222', '#87684f', '#45352d'], 14);
      ...
      stain.addColorStop(0, 'rgba(65, 54, 92, 0.34)');
      stain.addColorStop(0.62, 'rgba(29, 26, 43, 0.26)');
      stain.addColorStop(1, 'rgba(0,0,0,0)');
      ctx.fillStyle = stain;
      ctx.fillRect(0, baseY, TILE_W, TILE_H);
      ctx.strokeStyle = 'rgba(12, 14, 20, 0.42)';
      ctx.lineWidth = 1;
      for (let i = 0; i < 3; i++) {
        const y = baseY + 10 + rnd() * (TILE_H - 18);
        ctx.beginPath();
        ctx.moveTo(10 + rnd() * 12, y);
        ctx.lineTo(TILE_W - 16 - rnd() * 10, y + rnd() * 4 - 2);
        ctx.stroke();
      }
      ctx.restore();
      if (!withProps) break;
      ...
      for (let i = 0; i < n; i++) {
        ...
        const gy = baseY + 11 + rnd() * (TILE_H - 18);
        const r = 4.8 + rnd() * 4.2;
        const h = r * (1.75 + rnd() * 0.75);
        const pink = rnd() < 0.6;
        const col = pink ? '#ba59df' : '#46c4df';
        ctx.globalAlpha = 0.48;
        ctx.fillStyle = 'rgba(2, 6, 10, 0.74)';
        ctx.beginPath();
        ctx.ellipse(gx + 2.5, gy + 2.3, r * 1.35, r * 0.42, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.globalAlpha = 1;
        const g = ctx.createLinearGradient(gx - r, gy - h, gx + r, gy + r);
        g.addColorStop(0, '#dff8ff');
        g.addColorStop(0.3, shade(col, 0.12));
        g.addColorStop(0.74, shade(col, -0.2));
        g.addColorStop(1, '#22203a');
        ctx.fillStyle = g;
        ctx.beginPath();
        ctx.moveTo(gx, gy - h);
        ...
        ctx.closePath();
        ctx.fill();
        ctx.strokeStyle = 'rgba(186, 229, 241, 0.58)';
        ctx.lineWidth = 0.75;
        ctx.stroke();
        ...
        ctx.beginPath();
        ctx.ellipse(gx - r * 0.22, gy - h * 0.7, r * 0.1, r * 0.28, -0.45, 0, Math.PI * 2);
        ctx.fill();
      }
      break;
    }
    case Terrain.ROCK: {
      fillBase('#315432', '#1c3423');
      speckle(['#496d3f', '#1d3523', '#334d31'], 10);
      bevel('rgba(14,32,22,0.48)', 'rgba(8,24,20,0.56)', 'rgba(214,236,196,0.04)');
      const n = 3 + variant;
      ctx.save();
      diamond();
      ctx.clip();
      const ground = ctx.createRadialGradient(TILE_W / 2, baseY + TILE_HALF_H + 3, 3, TILE_W / 2, baseY + TILE_HALF_H, 34);
      ground.addColorStop(0, 'rgba(20, 21, 19, 0.32)');
      ...
      for (let i = 0; i < n; i++) {
        ...
        const r = 6.5 + rnd() * 8;
        const h = r * (0.9 + rnd() * 0.5);
        ctx.globalAlpha = 0.42;
        ctx.fillStyle = 'rgba(3, 6, 7, 0.72)';
        ctx.beginPath();
        ctx.ellipse(bx + r * 0.26, by + r * 0.18, r * 1.2, r * 0.42, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.globalAlpha = 1;
        const g = ctx.createLinearGradient(bx - r, by - h, bx + r, by + r * 0.4);
        g.addColorStop(0, '#aeb6ad');
        g.addColorStop(0.34, '#6f7971');
        g.addColorStop(0.68, '#424a48');
        g.addColorStop(1, '#23292b');
        ctx.fillStyle = g;
        ctx.beginPath();
        ctx.moveTo(bx - r * 0.95, by - r * 0.12);
        ctx.lineTo(bx - r * 0.6, by - h * 0.9);
        ctx.lineTo(bx - r * 0.08, by - h * 1.15);
        ctx.lineTo(bx + r * 0.42, by - h * 0.86);
        ctx.lineTo(bx + r * 1.04, by - r * 0.18);
        ctx.lineTo(bx + r * 0.68, by + r * 0.44);
        ctx.lineTo(bx - r * 0.2, by + r * 0.48);
        ctx.closePath();
        ctx.fill();
        ctx.strokeStyle = 'rgba(12, 15, 17, 0.84)';
        ctx.lineWidth = 0.9;
        ctx.stroke();
        ctx.strokeStyle = 'rgba(240,245,232,0.18)';
        ctx.beginPath();
        ctx.moveTo(bx - r * 0.46, by - h * 0.72);
        ctx.lineTo(bx - r * 0.08, by - h * 1.0);
        ctx.lineTo(bx + r * 0.34, by - h * 0.74);
        ctx.moveTo(bx - r * 0.02, by - h * 0.98);
        ctx.lineTo(bx - r * 0.12, by + r * 0.34);
        ctx.stroke();
      }
      break;
    }
    case Terrain.TREE: {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const ty = baseY + 20 + rnd() * (TILE_H - 24);
        const scale = 0.88 + rnd() * 0.28;
        ctx.globalAlpha = 0.36;
        ctx.fillStyle = 'rgba(2, 8, 7, 0.78)';
        ctx.beginPath();
        ctx.ellipse(tx + 5, ty + 2, 15 * scale, 5 * scale, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.globalAlpha = 1;
        const trunk = ctx.createLinearGradient(tx - 4, ty - 20, tx + 4, ty);
        trunk.addColorStop(0, '#74503a');
        trunk.addColorStop(1, '#2d2119');
        ctx.fillStyle = trunk;
        ctx.fillRect(tx - 2 * scale, ty - 20 * scale, 4 * scale, 21 * scale);
        for (let l = 0; l < 4; l++) {
          const canopyR = (15 - l * 1.8) * scale;
          const ly = ty - 8 * scale - l * 7.2 * scale;
          const lx = tx + (rnd() - 0.5) * 4;
          const g = ctx.createRadialGradient(lx - canopyR * 0.35, ly - canopyR * 0.42, 1, lx, ly, canopyR * 1.15);
          g.addColorStop(0, '#6fbf63');
          g.addColorStop(0.34, '#2f7639');
          g.addColorStop(1, '#0e3726');
          ctx.fillStyle = g;
          ctx.beginPath();
          ctx.moveTo(lx, ly - canopyR * 1.18);
          ctx.bezierCurveTo(lx + canopyR * 0.8, ly - canopyR * 0.72, lx + canopyR * 0.92, ly + canopyR * 0.1, lx + canopyR * 0.18, ly + canopyR * 0.48);
          ctx.bezierCurveTo(lx - canopyR * 0.58, ly + canopyR * 0.66, lx - canopyR * 0.98, ly, lx - canopyR * 0.76, ly - canopyR * 0.54);
          ctx.closePath();
          ctx.fill();
          ctx.strokeStyle = 'rgba(5, 23, 15, 0.58)';
          ctx.lineWidth = 0.6;
          ctx.stroke();
        }
        ctx.strokeStyle = 'rgba(218,255,199,0.15)';
        ctx.beginPath();
        ctx.moveTo(tx - 4, ty - 34 * scale);
        ctx.lineTo(tx - 10, ty - 14 * scale);
        ctx.stroke();
      }
      break;
    }
