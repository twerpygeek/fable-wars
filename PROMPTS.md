# Sprite Generation Prompts — every creature & structure

Copy-paste prompts for AI image generators (gpt-image, Midjourney, DALL-E, Stable Diffusion…).
Generate the PNG, then follow **"Saving your files"** at the bottom (or SPRITES.md for full detail).

## How to use

1. Take the **STYLE BLOCK** below, add one **SUBJECT** line from the tables, generate.
2. Ask for / ensure a **transparent background** (gpt-image: "transparent background";
   others: remove the background afterwards — remove.bg, Photoshop, etc.).
3. One front view per creature is enough to start — the game reuses it for every direction.
   For the full RA2 feel, also generate `sw, w, nw, n` views ("same creature seen from behind",
   etc.) and a second "mid-step" walking frame of each.
4. If a creature comes out looking *identical* to a real Pokémon, regenerate with
   "original creature design, legally distinct, not Pokémon" added. Parody-inspired is the goal;
   carbon copies are Nintendo's lawyers' goal.

### STYLE BLOCK — creatures (prepend to every unit prompt)

> chibi cartoon monster game sprite, viewed from a 3/4 high angle like an isometric real-time
> strategy game, single creature centered, full body, big round head with large glossy eyes,
> tiny body and stubby limbs, soft cel shading lit from the upper left, thin dark outline,
> vibrant saturated colors, clean vector-like game asset, transparent background, no text,
> no logo, no ground shadow

### STYLE BLOCK — buildings (prepend to every building prompt)

> isometric video-game building sprite at a true 2:1 isometric angle (Red Alert 2 style),
> single structure centered, base sits on a diamond-shaped footprint, soft cel shading lit
> from the upper left, thin dark outline, saturated colors, clean game asset, transparent
> background, no text, no ground plane outside the footprint

---

## 🔥 Scorch Legion creatures (13)

| File id | SUBJECT prompt |
|---|---|
...
## Buildings (45 = 15 types × 3 factions)

Faction material clauses — append the right one to each building prompt:
...
| File id (×3 factions) | SUBJECT prompt (add faction clause) |
|---|---|
| scorch_conyard / tide_conyard / verdant_conyard | a massive command headquarters with a fortified central tower, construction crane on the roof, glowing power core, the most important building in an RTS base |
| *_power | a power plant with two fat reactor coils wrapped in glowing energy rings, humming with elemental energy |
| *_refinery | an ore refinery with a tall storage silo, a conveyor belt, and a sparkling pile of pink and cyan candy crystals at its intake |
| *_barracks | a creature hatchery: a domed nest structure with three large speckled eggs nestled at its entrance |
| *_factory | a heavy war factory with a huge open garage door, rooftop crane, chimney vents, wide assembly hall |
| *_radar | a radar outpost with a large rotating dish antenna on a reinforced mast, blinking signal light |
| *_airpad | a flat landing pad with a glowing landing circle painted on it, small control hut and an orange windsock |
| *_navalyard | a naval drydock: two pier arms over open water with a gantry arch spanning them, mooring posts |
| *_techlab | a high-tech research lab with a glowing energy dome and two antenna spires, mysterious light pulsing inside |
| *_repair | a creature Care Center with a friendly pink roof, a white medical cross sign, and a small heart emblem over the door |
| *_wall | a single short segment of fortified wall, chunky and connectable, one tile wide |
| *_def_basic | a compact defensive turret: a rotating barrel cannon mounted on a low armored platform |
| *_def_adv | an advanced elemental defense tower: a tall pylon focusing a glowing energy orb at its tip, crackling with power |
| *_def_aa | an anti-air battery: a rack of three skyward-angled missile launchers on a swivel base, glowing missile tips |
...
Building sizes (make roughly this canvas, or just generate square and align on the template):
conyard & superweapons **200×180** · factory/navalyard/repair **200×160** · refinery **168×144** ·
power/barracks/radar/airpad/techlab **136×128** · defenses **72×96** · wall **72×58**.

---

## Saving your files (exact steps)

1. **Name and place the PNGs** (names must match exactly, all lowercase):
   - Creatures → `public/sprites/units/<id>_s_0.png` (front view).
     Optional extra views: `_sw_0`, `_w_0`, `_nw_0`, `_n_0`, and walking frames `_<dir>_1`.
   - Buildings → `public/sprites/buildings/<id>.png`
2. **Register them** in `public/sprites/manifest.json`:
   ```json
   {
     "units": {
       "scorch_peekachoo": { "facings": ["s"], "frames": 1 },
       "verdant_snorlux":  { "facings": ["s", "sw", "w", "nw", "n"], "frames": 2 }
     },
     "buildings": ["scorch_conyard", "tide_refinery"]
   }
   ```
3. **Check them**: `npm run dev`, then open `http://localhost:5173/?spritelab` — your art appears
   in place of the procedural sprites (icons update automatically). Play a match to see it live.
4. **Ship them**: `vercel deploy --prod` — then they're on https://fable-alert.vercel.app for everyone.

Tips: transparent background is required; the engine auto-scales any size (square works) and adds
the team-color band/banner itself, so don't paint team colors. For pixel-perfect building alignment,
download the template from `/?spritelab` and paste your art over it so the base sits on the dashed diamond.
